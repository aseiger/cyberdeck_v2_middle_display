...
class LCD_2inch4(lcdconfig.RaspberryPi):
    ...
    def reset(self):
        """Reset the display (ST7789) with datasheet-correct settle times.

        ST7789 timing requirements:
          * Reset pulse (RESX low) must be held comfortably above the 10us
            minimum — use 50ms to be safe.
          * After RESX is released (high) the controller needs >=120ms to
            finish its internal power-on / charge-pump init before it will
            reliably accept commands.

        The old code only waited 10ms after the final RST-high. That was a
        race: whenever the panel's internal boot ran long, the first init
        commands (0x11 Sleep-Out etc.) were dropped and the panel came up
        blank — intermittently. 120ms after release removes the race.
        """
        # Assert reset (RESX low)
        self.digital_write(self.RST_PIN, False)
        time.sleep(0.05)
        # Release reset (RESX high) and give the ST7789 time to be ready
    ...
    def Init(self):
        ...
        self.data(0x3C)#  NVCOM — raised from 0x33 for a brighter panel
        self.data(0x3F)#
